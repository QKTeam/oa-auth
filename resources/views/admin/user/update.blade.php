...
@section('content')
    <div style="text-align: center; width: 80%; margin-left: 10%;">
        ...
        <form action="api/user/update" method="put" id="update">
            ...
            <div class="form-group">
                ...
                <input type="email" class="form-control" id="email" placeholder="Email" disabled name="email" required value="{{$user['email']}}">
            ...
            <div class="form-group">
                <label for="password" style="display: block;">Admin Password</label>
    ...
    <script>
        $(document).ready(function(){
            var pattern = /^\d{11}$/
//            var pattern2 = /^(.*)?@(.*)?\.(.*)?$/
            $("#role{{$user['role_id']}}").attr('selected', "selected")
//            $("#email").blur(function () {
//                if ($('#email').val() === '') {
//                    $('#errorEmail').css('visibility', '')
//                } else if (!$('#email').val().match(pattern2)) {
//                    $('#errorEmail').css('visibility', '')
//                } else {
...
//                if ($('#email').val() === '' || !$('#email').val().match(pattern2)) {
//                    $('#email').focus()
//                    return
//                }
                if ($('#phone').val() !== '' && !$('#phone').val().match(pattern)) {
